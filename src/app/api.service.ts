...
export class ApiService {
  ...
  getPeople(includeInactive: boolean = false): Observable<Person[]> {
    return this.withRetry(
      this.http.get<Person[]>(`${API_URL}/people`, {
  ...
  createPerson(name: string): Observable<Person> {
    return this.withRetry(this.http.post<Person>(`${API_URL}/people`, { name }));
  }

  updatePerson(id: string, name: string): Observable<Person> {
    return this.withRetry(this.http.patch<Person>(`${API_URL}/people/${id}`, { name }));
  }

  deactivatePerson(id: string): Observable<Person> {
    return this.withRetry(this.http.delete<Person>(`${API_URL}/people/${id}`));
  }

  // Places endpoints
  getPlaces(includeInactive: boolean = false): Observable<Place[]> {
    return this.withRetry(
      this.http.get<Place[]>(`${API_URL}/places`, {
        params: { includeInactive: includeInactive.toString() }
      })
    );
  }
  ...
  createPlace(name: string): Observable<Place> {
    return this.withRetry(this.http.post<Place>(`${API_URL}/places`, { name }));
  }

  updatePlace(id: string, name: string): Observable<Place> {
    return this.withRetry(this.http.patch<Place>(`${API_URL}/places/${id}`, { name }));
  }

  deactivatePlace(id: string): Observable<Place> {
    return this.withRetry(this.http.delete<Place>(`${API_URL}/places/${id}`));
  }

  // Categories endpoints
  getCategories(includeInactive: boolean = false): Observable<Category[]> {
    return this.withRetry(
      this.http.get<Category[]>(`${API_URL}/categories`, {
        params: { includeInactive: includeInactive.toString() }
      })
    );
  }
  ...
  createCategory(name: string, sortOrder?: number): Observable<Category> {
    return this.withRetry(this.http.post<Category>(`${API_URL}/categories`, { name, sortOrder }));
  }

  updateCategory(id: string, name: string, sortOrder?: number): Observable<Category> {
    return this.withRetry(this.http.patch<Category>(`${API_URL}/categories/${id}`, { name, sortOrder }));
  }

  deactivateCategory(id: string): Observable<Category> {
    return this.withRetry(this.http.delete<Category>(`${API_URL}/categories/${id}`));
  }

  // Subcategories endpoints
  getSubcategories(categoryId: string, includeInactive: boolean = false): Observable<Subcategory[]> {
    return this.withRetry(
      this.http.get<Subcategory[]>(`${API_URL}/subcategories`, {
  ...
  createSubcategory(categoryId: string, name: string, sortOrder?: number): Observable<Subcategory> {
    return this.withRetry(this.http.post<Subcategory>(`${API_URL}/subcategories`, { categoryId, name, sortOrder }));
  }

  updateSubcategory(id: string, name: string, sortOrder?: number): Observable<Subcategory> {
    return this.withRetry(this.http.patch<Subcategory>(`${API_URL}/subcategories/${id}`, { name, sortOrder }));
  }

  deactivateSubcategory(id: string): Observable<Subcategory> {
    return this.withRetry(this.http.delete<Subcategory>(`${API_URL}/subcategories/${id}`));
  }
  ...
  getExpenses(filters?: {
    ...
    return this.withRetry(this.http.get<Expense[]>(`${API_URL}/expenses`, { params: filters ?? {} }));
  }

  createExpense(expense: Omit<Expense, 'id' | 'createdAt'>): Observable<Expense> {
    return this.withRetry(this.http.post<Expense>(`${API_URL}/expenses`, expense));
  }
}
